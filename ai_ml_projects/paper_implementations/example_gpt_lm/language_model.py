...
if __name__=="__main__":
    ...
    input_ids_dataset = torch.tensor(input_ids_dataset,dtype=torch.long).to(device)
    labels_dataset = torch.tensor(labels_dataset,dtype=torch.long).to(device)
    dataloader = DataLoader(TensorDataset(input_ids_dataset,labels_dataset))

    model = Model(vocab_size=16000,max_len=256,emb_dim=512,n_heads=8)
